...
from magdots.model.magn_lattice import FM_state, AFM_state
...
def main():
    ...
    a2 = np.array([1.0*a, 0.0*a, 0.0])


    Ha =  2.0*mu0*Ms
    na = np.array([0,0,1])

    dot = Dot(R=R, h=h, Ms=Ms, anisotropy=(na, Ha))
    ...
    result.save_to_file("edge_FM.npz")
