...
from magdots.model.dot import DoubleDot
...
def main():
    ...
    mu = np.array([0,0,1.0, 0.0, 0.0, -1.0])
    ...
    bulk = BulkWave(dot = dot, lattice = la, mu=mu,  Bbias = np.array([0, 0, 0, 0, 0,0]), K_MAX=20)


    # Setting points on the Brillouin zone to be calculated
    ...
    result.save_to_file("bulk_AFM.npz")
