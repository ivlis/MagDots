...
from magdots.util.util import  calculate_bulk, rotate
...
def main():
    ...
    R = 1.0
    a = 6.0*R
    a1 = np.array([1.0*a, 0.0, 0.0])
    a2 = rotate(a1, np.pi/3)
    Ms = 1.0
    h = 5*R
    ...
    theta = -np.pi/3
    Bbias_module = 0.16
    Bbias = Bbias_module*np.array([np.cos(theta)*np.sin(phi),np.sin(theta)*np.sin(phi),np.cos(phi)])


    dot = Dot(R=R, h=h, Ms=Ms)
    la = Lattice(a1=a1, a2=a2)
    bulk = BulkWave(dot = dot, lattice = la, mu=mu, Bbias = Bbias,  K_MAX=30)
    ...
    M2 = rotate(M, np.pi/3)
    M3 = rotate(M, 2*np.pi/3)

    
    K = X
    K2 = rotate(K, np.pi/3)
    K3 = rotate(K, 2*np.pi/3)
    K4 = rotate(K, np.pi)

    vertices = [G,K, M, K2, M2, K3, M3, K4, G]
    ...
    result.save_to_file("bulk_equilibrium.npz")


    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.plot(result.W[:,-1]/(gamma* mu0 *Ms), linewidth = 1.2, color='k')

    ax.set_xticks([n*points for n in range(0, len(vertices)+1)])
    ax.set_xticklabels(["$\Gamma$", "$K_1$", "$M_1$", "$K_2$", "$M_2$","$K_3$", "$M_3$", "$K_4$" ,"$\Gamma$"])
    ax.set_xlim(0, ((len(vertices)-1)*points))
